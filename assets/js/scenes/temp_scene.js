...
var cameraRTT, sceneRTT, sceneScreen,
	...
	camVerOrientation = new THREE.Vector3( 0, 0, 0 );
...
this.parameters = {
	...
	gamepad : {
		...
		optional: true,
		inputs: [
		    {name: "First GamePad Device"},
		    {name: "Second GamePad Device"}
		]
	},	
	midiDevice : {
		autoInit: false,
	...
	mediaDevice :{		
		...
		optional: false,
		inputs: [
		    {title: "Media Input 1st Request", name: "Front Camera", video: true, audio: false},
		    {title: "Media Input 2nd Request", name: "Back Camera", video: true, audio: false}
		]
	}
}
...
var camera = new THREE.PerspectiveCamera(75, aspect, 1, 1100);
var controls = new THREE.DeviceOrientationControls( camera );	

var vFOV = camera.fov * Math.PI / 180;        // convert vertical fov to radians
var wallHeight = (2 * Math.tan( vFOV / 2 )) * 5; // visible height
var wallWidth = wallHeight * aspect;                  // visible width
...
this.onHIDupdate = function(device){
	switch (device.HID) {
	    case 'MIDI':
	    ...
	    case 'GEOLOCATION':
	    	...
	    	console.log(device.position.coords);
...
this.render = function() {
	...
	camVerOrientation.applyQuaternion( camera.quaternion );

	plane.position.copy( camVerOrientation );
...
function createCombinedMaterial(){
	...
	// Preview Screen
	var planelikeGeometry = new THREE.CubeGeometry( wallWidth, wallHeight, 0.1 );
	plane = new THREE.Mesh( planelikeGeometry, new THREE.MeshBasicMaterial( { map: rtTexture } ) );
	scene.add(plane);
	//plane.visible = false;
	
	var screenMaterial = new THREE.MeshBasicMaterial({ color: 0x8000ff, transparent: true, opacity: 0.5 });	
	var screenGeo = new THREE.CubeGeometry( 600, 600, 1 );	
	var screenMesh = new THREE.Mesh( screenGeo, screenMaterial )
	
	// wireframe
	var helper = new THREE.EdgesHelper( screenMesh, 0xffffff ); // or THREE.WireframeHelper
	...
	screenMesh.position.set(0,-100,0);
}
